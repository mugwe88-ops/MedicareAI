          ...
          <div className="bg-gradient-to-r from-blue-900 via-indigo-900 to-blue-800 rounded-3xl p-6 md:p-8 text-white shadow-xl relative overflow-hidden">
            ...
            <div className="relative z-10 flex flex-col lg:flex-row lg:items-center justify-between gap-6">
              <div>
                ...
                <h2 className="text-2xl md:text-3xl font-black tracking-tight">Next Appointment: General Consultation</h2>
              ...
              <div className="flex items-center gap-3">
                <button 
                  ...
                  className="bg-emerald-500 hover:bg-emerald-600 text-white px-6 py-3.5 rounded-2xl font-black text-sm shadow-lg shadow-emerald-900/30 transition-all flex items-center gap-2"
                >
            ...
            <div className="space-y-6">
              <div className="bg-white rounded-3xl p-6 border border-slate-200/80 shadow-sm">
                ...
                <div className="flex items-center gap-4 mb-6">
                  <div className="w-16 h-16 rounded-2xl bg-blue-100 flex items-center text-blue-700 font-bold justify-center text-xl border border-blue-200">
                    DR
                  </div>
                  <div>
                ...
                <div className="space-y-4">
                  ...
                  <div className="pt-2 flex justify-end">
                    <button 
                      ...
                      className="bg-blue-600 hover:bg-blue-700 text-white px-6 py-3 rounded-xl text-xs font-bold shadow-md shadow-blue-200 transition-all"
                    >
                      Save & Submit Details
                    </button>
                  </div>
        ...
        <div className="fixed inset-0 z-50 bg-slate-950/90 backdrop-blur-md flex flex-col p-4 md:p-8 animate-fadeIn">
          ...
          <div className="flex items-center justify-between text-white pb-4 border-b border-slate-800">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h4 className="font-bold text-sm">Secure HD Video Call with Dr. Robert Vance</h4>
            ...
            <button 
              ...
              className="bg-red-600 hover:bg-red-700 text-white px-4 py-2 rounded-xl text-xs font-black shadow-lg shadow-red-900/50 flex items-center gap-1.5"
            >
              <AlertTriangle className="w-3.5 h-3.5" /> Emergency Button
            </button>
          </div>

          {/* Main Video Grid & AI Assistant Split */}
          <div className="flex-1 grid grid-cols-1 lg:grid-cols-4 gap-6 py-6">
            {/* Video Streams */}
            <div className="lg:col-span-3 grid grid-cols-1 md:grid-cols-2 gap-4 relative">
              {/* Doctor Video Box */}
              <div className="bg-slate-900 rounded-3xl border border-slate-800 flex flex-col items-center justify-center relative overflow-hidden shadow-2xl">
                ...
                <div className="w-24 h-24 rounded-full bg-blue-600/30 border border-blue-400/40 flex items-center text-blue-300 font-bold text-3xl mb-3 shadow-inner">
                  RV
                </div>
              ...
              <div className="bg-slate-900 rounded-3xl border border-slate-800 flex flex-col items-center justify-center relative overflow-hidden shadow-2xl">
                {isVideoOff ? (
                  ...
                  <>
                    <div className="absolute inset-0 bg-gradient-to-t from-slate-950/80 via-transparent to-transparent" />
                    <div className="w-24 h-24 rounded-full bg-slate-800 border border-slate-700 flex items-center text-slate-300 font-bold text-3xl mb-3">
                      WW
                    </div>
            ...
            <div className="bg-slate-900 rounded-3xl border border-slate-800 p-5 flex flex-col justify-between">
              <div>
                <div className="flex items-center gap-2 text-xs font-bold text-blue-400 uppercase tracking-wider mb-4">
                  <SparklesIcon className="w-4 h-4 text-blue-400" /> Live AI Note Assistant
                </div>
                <div className="space-y-3">
                  {aiNotes.map((note, idx) => (
                    <div key={idx} className="bg-slate-800/80 p-3 rounded-2xl border border-slate-700/50 text-xs text-slate-300">
                      • {note}
                    </div>
                  ))}
                </div>
              ...
              <div className="mt-4 pt-4 border-t border-slate-800 text-[10px] text-slate-400">
                AI transcribes key medical takeaways in real-time for your electronic records.
              </div>
            </div>
          </div>
          ...
          <div className="flex items-center justify-center gap-4 py-3">
            <button 
              ...
              className={`p-4 rounded-2xl text-white font-bold transition-all ${isMuted ? 'bg-red-600' : 'bg-slate-800 hover:bg-slate-700'}`}
            >
              <Mic className="w-5 h-5" />
            </button>
            <button 
              onClick={() => setIsVideoOff(!isVideoOff)}
              className={`p-4 rounded-2xl text-white font-bold transition-all ${isVideoOff ? 'bg-red-600' : 'bg-slate-800 hover:bg-slate-700'}`}
            >
              <Camera className="w-5 h-5" />
            </button>
            <button 
              onClick={() => setInCall(false)}
              className="bg-red-600 hover:bg-red-700 text-white px-6 py-4 rounded-2xl font-black text-sm shadow-lg flex items-center gap-2"
            >
        ...
        <div className="space-y-6 animate-fadeIn max-w-4xl mx-auto">
          <div className="bg-white rounded-3xl p-6 md:p-8 border border-slate-200/80 shadow-sm mb-6">
            ...
            <p className="text-xs text-slate-500 mt-1">
              Your complete health history recorded session-by-session. Especially valuable for managing chronic conditions like hypertension, diabetes, and asthma across multiple doctor visits.
            </p>
          </div>
          ...
          <div className="space-y-6 relative before:absolute before:inset-0 before:left-8 before:w-0.5 before:bg-slate-200">
            {consultationHistory.map((item, index) => (
              <div key={index} className="relative flex items-start gap-6 group">
                {/* Timeline Node Icon */}
                <div className="w-16 h-16 rounded-2xl bg-blue-600 text-white font-bold flex items-center justify-center flex-shrink-0 shadow-lg shadow-blue-200 z-10">
                  <FileText className="w-6 h-6" />
                </div>

                {/* Card Content */}
                <div className="flex-1 bg-white rounded-3xl p-6 border border-slate-200/80 shadow-sm hover:shadow-md transition-all">
