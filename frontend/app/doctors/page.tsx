...
export default function DoctorDashboardPage() {
  ...
  return (
    <div className="flex-1 flex flex-col p-3 sm:p-6 lg:p-8 space-y-4 sm:space-y-6 overflow-x-hidden bg-slate-50 w-full max-w-full">
      {/* TOP BAR */}
      <header className="flex flex-col sm:flex-row items-center justify-between gap-3 bg-white p-3.5 sm:px-6 sm:py-4 rounded-2xl sm:rounded-3xl border border-slate-200/80 shadow-sm w-full">
        <div className="relative w-full sm:w-96">
          <Search className="absolute left-3.5 top-3 text-slate-400" size={16} />
          <input
            type="text"
            placeholder="Search patients, invoice, appointments..."
            className="w-full bg-slate-50 border border-slate-200/80 rounded-xl sm:rounded-2xl pl-10 pr-3 py-2.5 text-xs font-medium text-slate-800 outline-none focus:border-blue-600 transition"
          />
        </div>

        <div className="flex items-center justify-between w-full sm:w-auto gap-3">
          <span className="sm:hidden text-xs font-black text-slate-400 uppercase tracking-wider">Actions</span>
          <div className="flex items-center gap-2">
            <button className="w-9 h-9 sm:w-10 sm:h-10 rounded-xl sm:rounded-2xl bg-slate-50 hover:bg-slate-100 border border-slate-200 text-slate-600 flex items-center justify-center transition shadow-sm cursor-pointer relative">
              <Bell size={16} />
              <span className="absolute top-1.5 right-1.5 w-2 h-2 bg-rose-500 rounded-full"></span>
            </button>
            <button className="w-9 h-9 sm:w-10 sm:h-10 rounded-xl sm:rounded-2xl bg-slate-50 hover:bg-slate-100 border border-slate-200 text-slate-600 flex items-center justify-center transition shadow-sm cursor-pointer">
              <Mail size={16} />
            </button>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-6 w-full">
            ...
            <div className="bg-white border border-slate-200/80 rounded-2xl sm:rounded-3xl p-4 sm:p-6 shadow-sm flex flex-col justify-between">
              ...
              <div className="flex flex-col items-center justify-center py-4">
                <div className="relative h-32 w-32 sm:h-36 sm:w-36 rounded-full border-8 border-slate-100 border-t-blue-600 flex flex-col items-center justify-center text-center shadow-inner">
                  <span className="text-[9px] sm:text-[10px] text-slate-400 font-bold uppercase">Practice Score</span>
                  <span className="text-2xl sm:text-3xl font-black text-slate-900 mt-0.5">492</span>
                </div>
        ...
        <div className="space-y-4 sm:space-y-6">
            ...
            {isLoading ? (
              ...
              <>
                <div className="flex flex-col items-center text-center space-y-2.5 pt-1">
                  {doctorInfo.avatar ? (
                    <div className="w-20 h-20 sm:w-24 sm:h-24 rounded-2xl sm:rounded-3xl bg-slate-100 overflow-hidden shadow-inner border border-slate-200">
                      <img src={doctorInfo.avatar} alt={doctorInfo.name} className="w-full h-full object-cover" />
                    </div>
                  ) : (
                    <div className="w-20 h-20 sm:w-24 sm:h-24 rounded-2xl sm:rounded-3xl bg-blue-600 text-white font-black text-xl sm:text-2xl flex items-center justify-center shadow-md">
                      {getInitials(doctorInfo.name)}
                    </div>
                  )}
                  <div>
                    <h3 className="text-sm sm:text-base font-black text-slate-900">{doctorInfo.name}</h3>
                ...
                <div className="space-y-2 pt-3 border-t border-slate-100 text-xs text-slate-600 font-medium">
                  <p className="flex justify-between items-center">
                    ...
                    <strong className="text-slate-900 text-right truncate max-w-[150px]">{doctorInfo.email}</strong>
                  </p>
                  <p className="flex justify-between items-center">
          ...
          <div className="bg-white border border-slate-200/80 rounded-2xl sm:rounded-3xl p-4 sm:p-6 shadow-sm space-y-4">
            <div className="flex items-center justify-between">
              <h3 className="text-[11px] sm:text-xs font-black uppercase tracking-wider text-slate-400">Appointments ({selectedDate})</h3>
