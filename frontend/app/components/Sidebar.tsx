...
export default function Sidebar() {
  ...
  // Online / Availability Toggle State
  ...
  return (
    <aside
      className={`relative flex flex-col h-screen bg-gradient-to-b from-slate-950 via-slate-900 to-slate-950 text-slate-300 border-r border-slate-800/80 shadow-2xl transition-all duration-300 ease-in-out select-none z-40 ${
    ...
    >
      ...
      <button
        ...
        className="absolute -right-3 top-7 w-6 h-6 bg-blue-600 hover:bg-blue-500 text-white rounded-full flex items-center justify-center shadow-lg border border-slate-900 transition cursor-pointer z-50"
      ...
      {/* QUICK NEW CONSULTATION FLOATING ACTION */}
      {!isCollapsed ? (
      ...
      {/* SYNC & SYSTEM STATUS (COLLAPSED/EXPANDED) */}
      {!isCollapsed && (
      ...
      {!isCollapsed && (
        <div className="p-3 mx-2.5 mb-2 bg-slate-900/90 border border-slate-800 rounded-2xl shadow-inner space-y-2.5">
          ...
          <button
            onClick={() => router.push("/doctors/telehealth/queue")}
